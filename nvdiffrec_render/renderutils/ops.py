...
class _fresnel_shlick_func(torch.autograd.Function):
    ...
    def forward(ctx, f0, f90, cosTheta):
        out = _C.fresnel_shlick_fwd(f0, f90, cosTheta, False)
    ...
    def backward(ctx, dout):
        ...
        return _C.fresnel_shlick_bwd(f0, f90, cosTheta, dout) + (None,)
...
class _ndf_ggx_func(torch.autograd.Function):
    ...
    def forward(ctx, alphaSqr, cosTheta):
        out = _C.ndf_ggx_fwd(alphaSqr, cosTheta, False)
    ...
    def backward(ctx, dout):
        ...
        return _C.ndf_ggx_bwd(alphaSqr, cosTheta, dout) + (None,)
...
class _lambda_ggx_func(torch.autograd.Function):
    @staticmethod
    def forward(ctx, alphaSqr, cosTheta):
        out = _C.lambda_ggx_fwd(alphaSqr, cosTheta, False)
        ctx.save_for_backward(alphaSqr, cosTheta)
        return out
    ...
    def backward(ctx, dout):
        alphaSqr, cosTheta = ctx.saved_variables
        return _C.lambda_ggx_bwd(alphaSqr, cosTheta, dout) + (None,)
...
class _masking_smith_func(torch.autograd.Function):
    ...
    def forward(ctx, alphaSqr, cosThetaI, cosThetaO):
        ...
        out = _C.masking_smith_fwd(alphaSqr, cosThetaI, cosThetaO, False)
        return out

    @staticmethod
    def backward(ctx, dout):
        alphaSqr, cosThetaI, cosThetaO = ctx.saved_variables
        return _C.masking_smith_bwd(alphaSqr, cosThetaI, cosThetaO, dout) + (None,)
...
class _prepare_shading_normal_func(torch.autograd.Function):
    ...
    def forward(ctx, pos, view_pos, perturbed_nrm, smooth_nrm, smooth_tng, geom_nrm, two_sided_shading, opengl):
        ...
        out = _C.prepare_shading_normal_fwd(pos, view_pos, perturbed_nrm, smooth_nrm, smooth_tng, geom_nrm, two_sided_shading, opengl, False)
    ...
    def backward(ctx, dout):
        ...
        return _C.prepare_shading_normal_bwd(pos, view_pos, perturbed_nrm, smooth_nrm, smooth_tng, geom_nrm, dout, ctx.two_sided_shading, ctx.opengl) + (None, None, None)
...
class _lambert_func(torch.autograd.Function):
    ...
    def forward(ctx, nrm, wi):
        out = _C.lambert_fwd(nrm, wi, False)
    ...
    def backward(ctx, dout):
        ...
        return _C.lambert_bwd(nrm, wi, dout) + (None,)
...
class _frostbite_diffuse_func(torch.autograd.Function):
    ...
    def forward(ctx, nrm, wi, wo, linearRoughness):
        out = _C.frostbite_fwd(nrm, wi, wo, linearRoughness, False)
    ...
    def backward(ctx, dout):
        ...
        return _C.frostbite_bwd(nrm, wi, wo, linearRoughness, dout) + (None,)
...
class _pbr_specular_func(torch.autograd.Function):
    ...
    def forward(ctx, col, nrm, wo, wi, alpha, min_roughness):
        ...
        out = _C.pbr_specular_fwd(col, nrm, wo, wi, alpha, min_roughness, False)
        return out

    @staticmethod
    def backward(ctx, dout):
        col, nrm, wo, wi, alpha = ctx.saved_variables
        return _C.pbr_specular_bwd(col, nrm, wo, wi, alpha, ctx.min_roughness, dout) + (None, None)
...
class _pbr_bsdf_func(torch.autograd.Function):
    ...
    def forward(ctx, kd, arm, pos, nrm, view_pos, light_pos, min_roughness, BSDF):
        ...
        ctx.min_roughness = min_roughness
        ctx.BSDF = BSDF
        out = _C.pbr_bsdf_fwd(kd, arm, pos, nrm, view_pos, light_pos, min_roughness, BSDF, False)
        return out

    @staticmethod
    def backward(ctx, dout):
        kd, arm, pos, nrm, view_pos, light_pos = ctx.saved_variables
        return _C.pbr_bsdf_bwd(kd, arm, pos, nrm, view_pos, light_pos, ctx.min_roughness, ctx.BSDF, dout) + (None, None, None)
...
class _diffuse_cubemap_func(torch.autograd.Function):
    ...
    def forward(ctx, cubemap):
        out = _C.diffuse_cubemap_fwd(cubemap)
    ...
    def backward(ctx, dout):
        ...
        cubemap_grad = _C.diffuse_cubemap_bwd(cubemap, dout)
...
class _specular_cubemap(torch.autograd.Function):
    ...
    def forward(ctx, cubemap, roughness, costheta_cutoff, bounds):
        out = _C.specular_cubemap_fwd(cubemap, bounds, roughness, costheta_cutoff)
    ...
    def backward(ctx, dout):
        ...
        cubemap_grad = _C.specular_cubemap_bwd(cubemap, bounds, dout, ctx.roughness, ctx.theta_cutoff)
...
def __ndfBounds(res, roughness, cutoff):
    ...
    bounds = _C.specular_bounds(res, costheta[idx])
...
class _image_loss_func(torch.autograd.Function):
    ...
    def forward(ctx, img, target, loss, tonemapper):
        ...
        out = _C.image_loss_fwd(img, target, loss, tonemapper, False)
        return out

    @staticmethod
    def backward(ctx, dout):
        img, target = ctx.saved_variables
        return _C.image_loss_bwd(img, target, dout, ctx.loss, ctx.tonemapper) + (None, None, None)
...
class _xfm_func(torch.autograd.Function):
    ...
    def forward(ctx, points, matrix, isPoints):
        ...
        return _C.xfm_fwd(points, matrix, isPoints, False)

    @staticmethod
    def backward(ctx, dout):
        points, matrix = ctx.saved_variables
        return (_C.xfm_bwd(points, matrix, dout, ctx.isPoints),) + (None, None, None)
